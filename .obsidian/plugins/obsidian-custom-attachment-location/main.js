...
var require_path = __commonJS({
  "node_modules/path/path.js"(exports2, module2) {
    ...
    posix4.normalize = function(path) {
      var isAbsolute = posix4.isAbsolute(path), trailingSlash = path && path[path.length - 1] === "/";
    ...
    posix4.isAbsolute = function(path) {
      return path.charAt(0) === "/";
    };
    posix4.join = function() {
      var path = "";
      for (var i = 0; i < arguments.length; i++) {
      ...
      return posix4.normalize(path);
    };
    posix4.relative = function(from, to) {
      from = posix4.resolve(from).substr(1);
      to = posix4.resolve(to).substr(1);
    ...
    posix4._makeLong = function(path) {
      return path;
    };
    posix4.dirname = function(path) {
    ...
    posix4.basename = function(path, ext) {
    ...
    posix4.extname = function(path) {
      return posixSplitPath(path)[3];
    };
    posix4.format = function(pathObject) {
      if (!util.isObject(pathObject)) {
        throw new TypeError(
          "Parameter 'pathObject' must be an object, not " + typeof pathObject
        );
      ...
      var dir = pathObject.dir ? pathObject.dir + posix4.sep : "";
      var base = pathObject.base || "";
      return dir + base;
    };
...
var CustomAttachmentLocationPluginSettings = class _CustomAttachmentLocationPluginSettings {
  ...
  toLowerCase = false;
  static load(data) {
    return _CustomAttachmentLocationPluginSettings.clone(data);
  }
...
var CustomAttachmentLocationPluginSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(this.containerEl).setName("Pasted File Name").setDesc("Available variables: ${filename}, ${date:format}, ${originalCopiedFilename}, ${prompt}.").addText((text) => text.setPlaceholder("file-${date:YYYYMMDDHHmmssSSS}").setValue(settings.pastedFileName).onChange(async (value) => {
    ...
    new import_obsidian.Setting(this.containerEl).setName("Automatically rename attachment folder").setDesc('When renaming md files, automatically rename attachment folder if folder name contains "${filename}".').addToggle((toggle) => toggle.setValue(settings.autoRenameFolder).onChange(async (value) => {
      settings.autoRenameFolder = value;
      this.display();
      await this.plugin.saveSettings(settings);
    }));
    new import_obsidian.Setting(this.containerEl).setName("Automatically rename attachment files").setDesc('When renaming md files, automatically rename attachment files if file name contains "${filename}".').addToggle((toggle) => toggle.setValue(settings.autoRenameFiles).onChange(async (value) => {
...
// src/AttachmentPath.ts
function interpolateToDigitRegex(template, targetFileName) {
...
async function interpolateDateToString(plugin, template, targetFileName, originalCopiedFilename) {
  const dateRegExp = /\$\{date:(.*?)\}/g;
...
async function getEarliestAttachmentFolder(plugin, attachmentFolderTemplate, targetFileName) {
  ...
  const folderStats = await Promise.all(folders.map(async (folder) => {
    const stat = await app.vault.adapter.stat(import_path.posix.join(app.vault.adapter.getBasePath(), folder.path));
    return {
      path: folder.path,
      ctime: stat?.ctime ?? 0
    };
...
async function getAttachmentFolderPath(plugin, noteFileName) {
  return await getEarliestAttachmentFolder(plugin, plugin.settings.attachmentFolderPath, noteFileName);
}
async function getAttachmentFolderFullPath(plugin, noteFolderPath, noteFileName) {
  let attachmentFolder = "";
  const useRelativePath = plugin.settings.attachmentFolderPath.startsWith("./");
  if (useRelativePath) {
    attachmentFolder = import_path.posix.join(noteFolderPath, await getAttachmentFolderPath(plugin, noteFileName));
  } else {
...
async function getPastedFileName(plugin, noteFileName, originalCopiedFilename) {
  ...
  async handle() {
    ...
    for (const entry of pastedEntries) {
      if (entry.textPromise) {
        ...
        let extension = import_path2.posix.extname(entry.file.name).slice(1);
        const originalCopiedFileName = import_path2.posix.basename(entry.file.name, "." + extension);
        let fileArrayBuffer;
...
// src/CustomAttachmentLocationPlugin.ts
var CustomAttachmentLocationPlugin = class extends import_obsidian5.Plugin {
  _settings;
  get settings() {
  ...
  addDateTimeFormat(str, dateTimeFormat) {
    return str.replaceAll("${date}", `\${date:${dateTimeFormat}}`);
  }
  async handleRename(newFile, oldFilePath) {
    ...
    const oldName = import_path3.posix.basename(oldFilePath, ".md");
    const oldMdFolderPath = import_path3.posix.dirname(oldFilePath);
    const oldAttachmentFolderPath = await getAttachmentFolderFullPath(this, oldMdFolderPath, oldName);
    const newAttachmentFolderPath = import_path3.posix.join(import_path3.posix.dirname(oldAttachmentFolderPath), newName);
    ...
    if (await this.app.vault.adapter.exists(oldAttachmentFolderPath) && oldAttachmentFolderPath !== newAttachmentFolderPath) {
      ...
      const newAttachmentParentFolderPath = import_path3.posix.dirname(newAttachmentFolderPath);
      ...
      const oldAttachmentParentFolderPath = import_path3.posix.dirname(oldAttachmentFolderPath);
      const oldAttachmentParentFolderList = await this.app.vault.adapter.list(oldAttachmentParentFolderPath);
    ...
    for (const embed of embeds) {
      ...
      if (link.endsWith(".png") || link.endsWith("jpeg")) {
        files.push(import_path3.posix.basename(link));
      } else {
        continue;
      }
    }
    const attachmentFiles = await this.app.vault.adapter.list(newAttachmentFolderPath);
    for (const file of attachmentFiles.files) {
      console.debug(file);
      const filePath = file;
      let fileName = import_path3.posix.basename(filePath);
      if (files.indexOf(fileName) > -1 && fileName.contains(oldName)) {
        fileName = fileName.replace(oldName, newName);
        const newFilePath = (0, import_obsidian5.normalizePath)(import_path3.posix.join(newAttachmentFolderPath, fileName));
        const file2 = this.app.vault.getAbstractFileByPath(filePath);
        if (file2 == null) {
          continue;
        }
        await this.app.fileManager.renameFile(file2, newFilePath);
      } else {
        continue;
      }
    }
  ...
  async handleDelete(file) {
    ...
    const mdFolderPath = import_path3.posix.dirname(file.path);
    ...
    const childFiles = [];
    import_obsidian5.Vault.recurseChildren(attachmentFolder, (child) => {
  ...
  async getAvailablePathForAttachments(filename, extension, file, originalFn) {
    ...
    const noteFolderPath = import_path3.posix.dirname(file.path);
    ...
    return this.app.vault.getAvailablePath(import_path3.posix.join(attachmentFolderFullPath, filename), extension);
  }
};
